...
import { deleteRateHike } from "./actions";

export default function DeleteRateHikeModal({ id, destination_code }: { id: string, destination_code: string }) {
  const router = useRouter();
  const handleDelete = async () => {
    const { error } = await deleteRateHike(id, destination_code);
    if (error) {
      toast.error(error);
      return;
    }
