...
export function AddRouteTable({ users }: { users: any }) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (client !== "") {
      ...
      router.refresh();
      router.push("/director/routes/targets");
      ...
      <form className="mt-4" onSubmit={handleSubmit}>
        <div className="mb-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 whitespace-nowrap text-sm">
            <p className="text-lg font-semibold tracking-tight">Client</p>
              ...
              <PopoverTrigger asChild>
                ...
                >
                  {client ? (
                    <>
                      {
                        users.find((user: any) => user.id === client)
                          ?.user_metadata.name
                      }{" "}
                      {users.find((user: any) => user.id === client)
                    ...
                    {users
                      .filter(
                        (item: any) =>
                          ...
                          item.user_metadata.role === "client",
                      )
                      .map((user: any) => (
                        <CommandItem
                          key={user.id}
                          onSelect={() => {
                            setClient(user.id === client ? "" : user.id);
                            setOpen(false);
                          }}
                        >
                          <Check
                            className={cn(
                              "mr-2 h-4 w-4",
                              client === user.id ? "opacity-100" : "opacity-0",
                            )}
                          />
