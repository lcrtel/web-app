...
export default async function Page({ params }: { params: { id: string } }) {
    ...
    let { data: targets, error } = await supabase
        .from("targets")
        .select("*")
        .match({ destination_code: route?.[0]?.destination_code })
        .eq("route_type", route?.[0]?.route_type);
            ...
            <div className="">
                <div>
                    ...
                    <div className="flex justify-between items-center mb-5">
                        ...
                        <Link
                            href={`/user/routes/offers/post/${route?.[0]?.id}`}
                            className=""
                        >
                ...
                <div className="grid sm:grid-cols-2 gap-4 bg-surface rounded-lg p-4 mb-5">
                    ...
                    <div className="w-full flex justify-between items-center bg-white rounded-md px-2 py-1">
                        ...
                        <p className=" font-semibold">
                            {formatString(route?.[0]?.verification)}
                        </p>
                    </div>
                    <div className="w-full flex justify-between items-center bg-white rounded-md px-2 py-1">
