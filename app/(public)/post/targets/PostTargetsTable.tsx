...
export function PostRouteTable() {
  ...
  useEffect(() => {
    const storedRouteData = localStorage.getItem("pendingBuyingTargetsData");
    if (storedRouteData) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const { data: route, error } = await supabase.from("targets").insert(
      data.map((route: any) => ({
  ...
  const ImportDropdown = () => {
    ...
    return (
      <div className="relative text-left">
        <div
          onClick={handleCLick}
          className="hover:bg-primary text-primary relative flex cursor-pointer items-center justify-center rounded-lg border px-3 py-2 text-sm font-medium shadow-sm transition-all ease-in-out hover:bg-opacity-5"
        >
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-8xl p-4 md:px-8 md:py-5">
      <Link
        href="/"
        className="mb-2 mt-3 inline-flex items-center text-gray-400 transition-all ease-in-out hover:text-primary-900"
      >
        <HiOutlineArrowCircleLeft className="mr-1.5" /> Go back to home
      </Link>
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-primary text-lg font-bold tracking-tight md:text-2xl">
          Post your targets!
        </h3>
        <ImportDropdown />
      </div>
