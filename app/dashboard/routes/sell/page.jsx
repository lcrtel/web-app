...
export default async function Page() {
    ...
    const { data: routes } = await supabase
        .from("routes")
        .select("*")
        .match({ status: "for_sale", seller_id: session.user.id });
    return (
        <div className="">
            {userRole === "seller" ? (
                <>
                    <div className="flex mb-4 justify-between items-center">
                        ...
                        {routes.length ? (
                            <Link
                                ...
                                className="flex items-center gap-2 justify-center rounded-xl bg-primary-500 px-3 py-2 text-sm font-medium text-white "
                            >
                        ...
                        <div className="flex-1 border rounded-xl p-4">
                            <div className="flex p-10 flex-col gap-2 items-center justify-center">
                                <div className="bg-primary-500 bg-opacity-5 flex items-center justify-center p-2 rounded-full">
                                    <HiOutlineSearch className="w-10 h-10 rounded-full p-2 text-primary-500 bg-primary-500 bg-opacity-10" />
                                </div>
